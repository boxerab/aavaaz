...
```
`--batch-max-queue-wait` (default 0.5 seconds) turns new clients away with a WAIT message once requests sit in the batch queue longer than that. `--batch-max-admissions-per-s` (default 5) caps how many new clients get in per second, so a burst of reconnects cannot overshoot the queue before the wait reflects it. `--batch-beam-size` (default 1, greedy) sets the decoder beam width, 5 matches the non-batched path at about 2.5x the decode time. `--batch-temperature-fallback` (default off) re-decodes chunks that fail the quality check at higher temperatures, each such pass costs about as much as the first and halves throughput at beam 1.
